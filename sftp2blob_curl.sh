...
stream_file_to_blob_v3() {
    ...
    eval "$full_command" 2>/dev/null | {
        ...
        while true; do
            ...
            # Ensure the "Uploading chunk" message is printed only once
            echo "Uploading chunk with Block ID $BLOCK_ID (Size: $chunk_size_uploaded bytes)..."
            ...
            # Append BLOCK_ID to the block list file after successful upload
            echo "<Latest>$BLOCK_ID</Latest>" >> "$block_list_file"
...
stream_file_to_blob_v3 "$access_token" "$AZURE_STORAGE_ACCOUNT" "$AZURE_CONTAINER_NAME" "$AZURE_BLOB_NAME"
